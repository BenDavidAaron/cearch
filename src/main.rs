        ...
        Commands::Index { force: _ } => {
            ...
            match index::list_git_tracked_files(&root) {
                Ok(files) => {
                    ...
                    // Set up multi-progress with a main bar for files and a transient per-file bar
                    ...
                    if let Ok(style) = ProgressStyle::with_template(
                        ...
                        main_pb.set_style(style.progress_chars("=> "));
                    }
                    main_pb.set_message(String::from("Indexing repo"));
                    ...
                    for f in files {
                        ...
                        let file_pb = mp.add(ProgressBar::new(symbols_in_file.len() as u64));
                        if let Ok(style) = ProgressStyle::with_template(
                        ...
                        if let Some(name) = f.file_name().and_then(|s| s.to_str()) {
                            file_pb.set_message(name.to_string());
                        }
